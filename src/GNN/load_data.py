...
def load_data(args):
    ...
    k = args.k
    print(1)
    drug_drug = pd.read_csv('{}/drug_drug.csv'.format(dataset), header=None)
    print(1)
    drug_drug_link = topk_filtering(drug_drug.values, k)
    disease_disease = pd.read_csv('{}/disease_disease.csv'.format(dataset), header=None)
    disease_disease_link = topk_filtering(disease_disease.values, k)
    drug_disease = pd.read_csv('{}/drug_disease.csv'.format(dataset), header=None)
    ...
    if os.path.exists('{}_temp_{}k'.format(args.dataset, args.k)):
